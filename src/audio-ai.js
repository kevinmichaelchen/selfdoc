import { AUDIO_CHANGED } from './audio.js';
import { alignTranscript, mergeSkips } from './audio-align.js';

/**
 * Author-side speech model, dev only: whisper-tiny.en via Transformers.js,
 * downloaded once (~40 MB, cached by the browser) and run on the author's
 * machine at save time. Its output — per-word timestamps and filler skips —
 * lands in the take's meta beside the trim bounds, so readers, exports, and
 * the deployed site stay completely model-free.
 */
...
export async function alignTake(slug, key, blob, bounds, targetTokens) {
  if (import.meta.env.DEV) {
    try {
      ...
      asrPromise ??= pipeline('automatic-speech-recognition', 'onnx-community/whisper-tiny.en_timestamped', {
